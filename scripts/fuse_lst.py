...
def main() -> int:
    ...
    daily_min = float(ds["lst_daily"].min())
    daily_max = float(ds["lst_daily"].max())
    print(f"  lst_daily range: {daily_min:.1f} … {daily_max:.1f} °C")
    amp_min = float(ds["diurnal_amp"].min())
    amp_max = float(ds["diurnal_amp"].max())
    print(f"  diurnal_amp range: {amp_min:.1f} … {amp_max:.1f} °C")
